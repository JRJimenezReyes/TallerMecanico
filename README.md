...
1. Crea el paquete `evento` y pasa el enumerado `Opcion` a este paquete. Renombra dicho enumerado a `Evento` y haz que los métodos se adecuen a lo expresado en el diagrama de clases.
...
1. Adecúa la clase `Controlador` a lo expresado en el diagrama. Deberá suscribirse al gestor de evento de la vistaTexto para todos los evento y en el método `actualizar` los tratará interactuando con la vistaTexto y con el modeloCascada para llevar a cabo las diferentes operaciones.
